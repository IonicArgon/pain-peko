...
Chassis::Chassis(
    ...
    pros::motor_gearset_e       prm_cart
      ...
      m_trk_l {std::abs(prm_trk_l), std::abs(prm_trk_l) + 1, (prm_trk_l > 0) ? false : true},
      m_trk_r {std::abs(prm_trk_r), std::abs(prm_trk_r) + 1, (prm_trk_r > 0) ? false : true}
{
    set_brake(prm_brake);
    set_unit(prm_unit);
    set_cart(prm_cart);
...
int Chassis::get_trk(char prm_trk_side)
{
    ...
    {
    case 'l':
        return m_trk_l.get_value();
    case 'r':
        return m_trk_r.get_value();
    }
}